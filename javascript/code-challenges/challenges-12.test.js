...
const count = (target, input) => {
  ...
  const array = input.reduce((acc, curr) => acc.concat(curr), []);

  const repeats = array.reduce((count, num) => (num === target ? count + 1 : count), 0);
...
const totalSum = (input) => {
  const sum = input.reduce((acc, curr) => acc + curr.reduce((innerAcc, innerCurr) => innerAcc + innerCurr, 0), 0);
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  const result = input.map((arr) => {
    const filteredArr = arr.filter((num) => isANumber(num) && divisibleByFive(num));
